    ...
    <div class="w-full max-w-4xl">
      ...
      <router-link
          ...
          class="mb-6 sm:mb-8 inline-flex items-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3
               bg-white/60 backdrop-blur-sm border border-white/40 rounded-full
      ...
      <!-- Loading -->
      <div v-if="loading" class="bg-white/70 backdrop-blur-md border border-white/40
                                  rounded-2xl sm:rounded-3xl shadow-sm p-6 sm:p-12 text-center">
        <p class="text-gray-500">Analyse wird durchgeführt...</p>
      </div>

      <!-- Analyse Panel -->
      <div v-else class="bg-white/70 backdrop-blur-md border border-white/40
                          rounded-2xl sm:rounded-3xl shadow-sm p-6 sm:p-12">
        ...
        <div class="space-y-4 sm:space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            <router-link
                ...
                class="group bg-white/50 border border-white/30 rounded-xl sm:rounded-2xl
                     p-4 sm:p-6 hover:bg-white/70 transition-all text-left"
            >
              <div class="flex items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
                <div class="w-12 h-12 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl
                            bg-white/40 flex items-center justify-center text-2xl sm:text-3xl">
                  {{ result.emoji }}
                </div>
                <div>
              ...
              <p class="text-sm sm:text-base text-gray-600 leading-relaxed">{{ result.description }}</p>
        ...
        <!-- Bibliothek Link -->
        <div class="mt-8 sm:mt-10 pt-6 sm:pt-8 border-t border-white/30">
          <router-link
              to="/library"
              class="block w-full py-3 sm:py-4 bg-white/50 border border-white/30
                   rounded-full hover:bg-white/70 transition-all
                   text-gray-700 text-base sm:text-lg text-center"
          >
