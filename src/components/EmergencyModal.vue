<template>
  <Transition name="modal-fade">
    ...
    >
      ...
      <!-- Modal -->
      <div
        class="relative w-full sm:max-w-lg bg-white rounded-t-3xl sm:rounded-3xl z-10 overflow-hidden max-h-[92vh] flex flex-col"
      >
        ...
        <div class="flex items-start justify-between px-6 pt-4 pb-3 sm:px-8 sm:pt-6 sm:pb-4 shrink-0">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-gray-400 mt-0.5">Du bist nicht allein</p>
            </div>
          </div>
          <button
            @click="close"
            class="p-2 bg-gray-100 rounded-full hover:bg-gray-200 transition-colors shrink-0 mt-0.5"
          >
        ...
        <div class="overflow-y-auto flex-1 px-6 pb-6 sm:px-8 sm:pb-8 space-y-4">
          ...
          <div class="bg-red-50 border border-red-100 rounded-2xl p-4 flex items-center gap-4">
            ...
            <div class="flex-1 min-w-0">
              <p class="text-xs font-medium text-red-400 uppercase tracking-wide mb-0.5">Krisentelefon · 24/7 kostenlos</p>
              <a
          ...
          <div class="bg-gray-50 border border-gray-100 rounded-2xl p-4">
            <div class="flex items-center gap-2 mb-3">
              <WindIcon class="w-4 h-4 text-blue-400" />
            ...
            <div class="flex flex-col items-center py-2">
              <div class="relative flex items-center justify-center mb-3" style="width: 112px; height: 112px;">
                <div
                  class="absolute rounded-full border-2 border-blue-200"
                  :style="breathingActive ? breathingRingStyle : 'width:56px;height:56px;opacity:0;transition:all 1s ease'"
                />
                <div
                  class="rounded-full bg-blue-100 border-2 border-blue-300 flex items-center justify-center"
                  :style="breathingActive ? breathingCircleStyle : 'width:56px;height:56px;transition:all 1s ease'"
                >
                  <span class="text-xs font-bold text-blue-500 select-none">{{ breathingActive ? breathingCount : '·' }}</span>
                </div>
              </div>
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-2">
              <div
                ...
                :key="i"
                class="bg-white border border-gray-100 rounded-2xl p-3 hover:border-gray-200 hover:shadow-sm transition-all"
              >
                <div class="text-lg mb-1.5">{{ tip.emoji }}</div>
                <p class="text-xs font-semibold text-gray-700 leading-tight mb-0.5">{{ tip.title }}</p>
                <p class="text-xs text-gray-400 leading-snug">{{ tip.desc }}</p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="space-y-2">
              <a
                ...
                class="flex items-center gap-3 bg-white border border-gray-100 rounded-2xl p-3 hover:border-gray-300 hover:shadow-sm transition-all group"
              >
                <div
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-xs text-gray-400 truncate">{{ link.subtitle }}</p>
                </div>
                <ExternalLinkIcon class="w-3.5 h-3.5 text-gray-300 group-hover:text-gray-500 transition-colors shrink-0" />
              </a>
            </div>
          </div>
...
function toggleGrounding(i) {
  groundingDone.value[i] = !groundingDone.value[i]
}
...
const resourceLinks = [
  {
    ...
    subtitle: 'telefonseelsorge.de · kostenlos & anonym',
  ...
  },
  {
    ...
    subtitle: 'online.telefonseelsorge.de',
  ...
  },
  {
    ...
    subtitle: 'deutsche-depressionshilfe.de',
    url: 'https://www.deutsche-depressionshilfe.de',
  },
  {
    emoji: '🏥',
    bg: 'bg-green-50',
    title: 'Psychiatrie-Notaufnahme',
    subtitle: 'Nächste psychiatrische Klinik finden',
    url: 'https://www.google.com/search?q=psychiatrische+notaufnahme+in+meiner+nähe',
  },
]
...
.modal-fade-enter-active {
  animation: modalSlideUp 0.35s cubic-bezier(0.34, 1.1, 0.64, 1);
}
